      ...
      <div>
        <div className='admin-sidebar-header'>
          <div className='logo-img'>
            {isOpen ? <img src='/Logo6.jpg' alt='logo' className='h-[30px] w-[40px]' /> : <img src='/L1.png' alt='logo1' className='h-[32px] w-[32px]' />}
          </div>
        </div>
        <nav>
          {linksToRender.map((link) => {
            ...
            return (
              <Link key={link.path} to={link.path} className={`sidebar-link ${isActive(link.path) ? 'active' : ''}`} data-tooltip={link.label}>
                <div className='dashboard-icon'>
                  <link.icon style={{ width: '18px', flexShrink: 0 }} />{' '}
                  <span className='link-text'>{link.label}</span>
                </div>
                {link.isNotification && newRequestCount > 0 && (<span className='notification-badge'>{newRequestCount}</span>)}
              </Link>
            );
          })}
