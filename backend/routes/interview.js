...
router.get('/', async (req, res) => {
  try {
    const pipeline = [
      ...
      { $project: { _id: 1, candidateId: 1, jobId: 1, status: 1, companyId: 1, userId: 1, date: 1, interviewLevel: 1, createdAt: 1, updatedAt: 1, candidateName: '$candidateInfo.name', companyName: '$companyInfo.name', jobRole: '$jobInfo.role' } },
      { $sort: { date: -1 } },
    ];
...
router.get('/all', async (req, res) => {
  try {
    const pipeline = [
      { $match: { approvalStatus: { $in: ['pending', 'approved', 'rejected'] } } },
      { $addFields: { candidateObjectId: { $toObjectId: '$candidateId' }, jobObjectId: { $toObjectId: '$jobId' }, userObjectId: { $toObjectId: '$userId' } } },
      ...
      { $lookup: { from: 'companies', localField: 'companyId', foreignField: '_id', as: 'companyInfo' } },
      { $unwind: { path: '$companyInfo', preserveNullAndEmptyArrays: true } },
      { $project: { _id: 1, candidateId: 1, jobId: 1, status: 1, companyId: 1, userId: 1, date: 1, interviewLevel: 1, approvalStatus: 1, createdAt: 1, updatedAt: 1, userName: '$userDetails.name', candidateName: '$candidateInfo.name', companyName: '$companyInfo.name', jobRole: '$jobInfo.role' } },
      { $sort: { updatedAt: -1 } },
    ];
    const result = await Interview.aggregate(pipeline);
    res.status(200).json(result);
    ...
    res.status(500).json({ message: 'Server error while fetching all interviews.' });
  }
});
...
router.post('/', async (req, res) => {
  ...
  try {
    const interviewExists = await Interview.findOne({ candidateId, jobId, interviewLevel });
    if (interviewExists) {
      return res.status(409).json({ message: `An interview for level ${interviewLevel} already exists for this candidate and job.` });
    }

    const job = await Job.findById(jobId);
    if (!job || job.companyId.toString() !== companyId) {
        return res.status(400).json({ message: 'This job ID does not belong to the selected company.' });
    }
    ...
    const message = `${userName || 'A user'} scheduled a new L${interviewLevel} interview that needs your approval.`;
    const notification = new Notification({
        title: 'Interview Approval Required', message, type: 'warning', link: '/admin/interviews/approvals'
    });
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        const updatedInterview = await Interview.findByIdAndUpdate(id, { $set: updateData }, { new: true });
        if (!updatedInterview) {
        ...
        if (candidate && job && company) {
            ...
            if (updateData.status) {
                ...
                if (recruiter) io.to(recruiter.role).emit('interviewStatusUpdated', { message });
                io.to('manager').emit('interviewStatusUpdated', { message });
            } else if (updateData.approvalStatus) {
                title = `Interview ${updateData.approvalStatus.charAt(0).toUpperCase() + updateData.approvalStatus.slice(1)}`;
                message = `The interview for ${candidate.name} was ${updateData.approvalStatus} by ${approverName || 'a manager'}.`;
                if (recruiter) io.to(recruiter.role).emit('interviewStatusUpdated', { message });
            }

            if (title && message) {
                const notification = new Notification({ title, message, type: 'info', link: '/admin/interviews' });
                await notification.save();
            }
        }
