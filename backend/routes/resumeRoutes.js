...
const ATS_FRIENDLY_PATTERNS = {
  ...
  PROPER_HEADINGS: /(experience|education|skills|summary|work\s*history|professional)/i,
...
const SKILLS_DATABASE = {
  HARD_SKILLS: {
    technical: [
      'javascript', 'python', 'java', 'html', 'css', 'react', 'angular', 'vue', 'node.js',
      'express', 'django', 'flask', 'spring', 'sql', 'mysql', 'postgresql', 'mongodb',
      'aws', 'azure', 'gcp', 'docker', 'kubernetes', 'jenkins', 'git', 'github', 'gitlab',
      'typescript', 'php', 'c++', 'c#', 'ruby', 'go', 'swift', 'kotlin', 'rust',
      'rest api', 'graphql', 'microservices', 'ci/cd', 'devops', 'agile', 'scrum'
    ],
    business: [
      'financial analysis', 'budgeting', 'forecasting', 'market research', 'data analysis',
      'project management', 'risk management', 'strategic planning', 'business development',
      'sales', 'marketing', 'digital marketing', 'seo', 'sem', 'social media', 'content marketing',
      'accounting', 'quickbooks', 'excel', 'powerpoint', 'word', 'outlook'
    ],
    healthcare: [
      'patient care', 'medical terminology', 'emr', 'ehr', 'hipaa', 'clinical', 'phlebotomy',
      'cpr', 'bls', 'acls', 'nursing', 'medical coding', 'icd-10', 'healthcare', 'vitals'
    ],
    creative: [
      'photoshop', 'illustrator', 'indesign', 'figma', 'sketch', 'adobe xd', 'ui/ux',
      'graphic design', 'web design', 'motion graphics', 'video editing', 'premiere pro',
      'after effects', 'final cut pro', 'blender', 'maya'
    ]
  },
  SOFT_SKILLS: [
    'communication', 'leadership', 'teamwork', 'collaboration', 'problem solving', 'critical thinking',
    'adaptability', 'time management', 'organization', 'creativity', 'innovation', 'work ethic',
    'emotional intelligence', 'conflict resolution', 'negotiation', 'presentation', 'public speaking',
    'analytical', 'strategic', 'decision making', 'mentoring', 'coaching', 'training'
  ]
};
...
const INDUSTRY_SKILL_PRIORITIES = {
  technology: {
    hard: ['javascript', 'python', 'react', 'node.js', 'sql', 'aws', 'git', 'docker'],
    soft: ['problem solving', 'communication', 'teamwork', 'adaptability']
  },
  business: {
    hard: ['project management', 'excel', 'financial analysis', 'marketing', 'sales'],
  ...
  education: {
    hard: ['curriculum development', 'lesson planning', 'classroom management', 'educational technology'],
    soft: ['communication', 'patience', 'leadership', 'creativity']
  },
  general: {
    hard: ['microsoft office', 'communication', 'organization', 'project management'],
...
const detectATSFormatCompatibility = (text, extractedData) => {
  ...
  const hasComplexFormatting = text.match(/column|table|text\-box|header|footer/i);
  ...
  if (foundSections.length >= 3) {
    ...
    formatIssues.push(`Add missing sections: ${expectedSections.filter(s => !foundSections.includes(s)).join(', ')}`);
  }

  // 3. Check for proper headings - 20 points
  const hasStandardHeadings = /(experience|education|skills|work history|professional)/i.test(text);
  if (hasStandardHeadings) {
    ...
    formatIssues.push('Use standard section headings (Experience, Education, Skills)');
  }
  ...
  const contactInfoScore = (extractedData.email ? 3 : 0) + (extractedData.phone ? 3 : 0) + (extractedData.name ? 2 : 0);
...
const detectSkillsWithContext = (text, industry = 'general') => {
  ...
  const industryPriority = INDUSTRY_SKILL_PRIORITIES[industry] || INDUSTRY_SKILL_PRIORITIES.general;
  ...
  Object.values(SKILLS_DATABASE.HARD_SKILLS).flat().forEach(skill => {
  ...
  let normalizedHardScore = Math.min(95, (hardSkillsScore / Math.max(1, maxHardSkills)) * 100);
  let normalizedSoftScore = Math.min(95, (softSkillsScore / Math.max(1, maxSoftSkills)) * 100);

  // Add realistic variation
  normalizedHardScore = Math.max(45, normalizedHardScore + (Math.random() * 10 - 5));
  normalizedSoftScore = Math.max(45, normalizedSoftScore + (Math.random() * 10 - 5));

  return {
    hardSkills: {
      score: Math.round(normalizedHardScore),
      found: [...new Set(foundHardSkills)].slice(0, 20),
      missing: industryPriority.hard.filter(skill => 
    ...
    softSkills: {
      ...
      missing: industryPriority.soft.filter(skill => 
...
const analyzeResumeStructure = (text, extractedData) => {
  ...
  const actionVerbs = [
    ...
    'increased', 'reduced', 'achieved', 'delivered', 'transformed'
  ];
...
const analyzeContentQuality = (text, extractedData) => {
  ...
  const achievementIndicators = [
    ...
    'saved', 'generated', 'developed', 'created', 'implemented'
  ];
  ...
  const hasNumbers = /\d+%|\$\d+|\d+\s*(years|months)|increased by|decreased by/i.test(text);
  if (hasNumbers) {
...
const getGradeFromScore = (score) => {
  ...
  if (score >= 70) return { grade: 'B', label: 'Above Average', color: '#3b82f6' };
  if (score >= 67) return { grade: 'B-', label: 'Average', color: '#8b5cf6' };
  if (score >= 63) return { grade: 'C+', label: 'Below Average', color: '#f59e0b' };
  if (score >= 60) return { grade: 'C', label: 'Needs Improvement', color: '#f97316' };
...
const calculateRealisticScore = (analysisData) => {
  ...
  const industryMultiplier = industry === 'technology' ? 1.02 : 
                           industry === 'business' ? 1.01 : 1.0;
...
const analyzeResumeComprehensive = (resumeText) => {
  ...
  const phoneMatch = text.match(/(\+?\d{1,3}[-.\s]?)?(\(?\d{3}\)?[-.\s]?)?[\d\s-]{7,10}/);
  ...
  // Industry detection
  const industryTerms = {
    technology: ['software', 'developer', 'engineer', 'programming', 'code', 'technical', 'it'],
    business: ['business', 'management', 'administrative', 'office', 'corporate', 'finance', 'marketing'],
    healthcare: ['medical', 'health', 'patient', 'clinical', 'hospital', 'nursing', 'healthcare'],
    education: ['teacher', 'education', 'student', 'learning', 'academic', 'curriculum']
  };
  ...
  // Job level detection
  const seniorTerms = ['senior', 'lead', 'principal', 'manager', 'director', 'head'];
...
// Upload configuration (same as before)
const upload = multer({ 
...
router.post('/analyze', (req, res) => {
  ...
  upload(req, res, async (err) => {
    if (err) {
      ...
      const message = err.message.includes('format') 
    ...
    try {
      ...
      console.log('✅ File downloaded, buffer size:', fileBuffer.length, 'bytes');
      ...
      } else if (req.file.mimetype === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || req.file.path.endsWith('.docx')) {
      ...
      if (!extractedText || extractedText.trim().length === 0) {
        throw new Error('No text could be extracted. The file might be corrupted, password-protected, or contain only images.');
      }
      ...
      res.status(500).json({ 
        success: false,
        message: error.message || 'Could not parse the uploaded file on the server.'
      });
      ...
      if (req.file && req.file.filename) {
        ...
        cloudinary.uploader.destroy(req.file.filename, { resource_type: 'raw' })
          ...
          .catch(delErr => console.error('⚠️ Cleanup failed:', delErr.message));
      }
    }
  });
});
...
router.get('/history', async (req, res) => {
  try {
    const analyses = await ResumeAnalysis.find()
      .sort({ createdAt: -1 })
      .limit(10)
      .select('originalFilename atsScore overallScore grade gradeLabel createdAt industry jobLevel');
...
router.get('/health', (req, res) => {
  res.json({ 
    ...
    features: ['realistic_scoring_67_90', 'grading_system', 'industry_context', 'enhanced_recommendations']
  });
});
