...
export const NotificationProvider = ({ children }) => {
    ...
    const playSound = useCallback(() => {
        if (!audioUnlockedRef.current) {
            console.warn("Audio not unlocked yet. Click anywhere on the page to enable sound.");
            return;
        }
        try {
    ...
    useEffect(() => {
        ...
        const events = {
            'newInfoRequest': { title: 'New Candidate Request', type: 'info', link: '/admin/view-requests' },
            'newApplication': { title: 'New Job Application', type: 'success', link: '/admin/applications' },
            'newEnrollment': { title: 'New Enrollment', type: 'info', link: '/admin/studentenrollment' },
            'newFormSubmission': { title: 'New Contact Message', type: 'warning', link: '/admin/forms' },
            'newJobPosting': { title: 'New Job Posting', type: 'info', link: '/admin/manage-jobs' },
            'newCandidateAdded': { title: 'New Candidate Added', type: 'success', link: '/admin/manage-candidates' },
            'newInterview': { title: 'New Interview Scheduled', type: 'success', link: '/admin/interviews' },
            'interviewStatusUpdated': { title: 'Interview Status Updated', type: 'info', link: '/admin/interviews' },
        };
        ...
        Object.entries(events).forEach(([eventName, details]) => {
            socket.on(eventName, (data) => {
                handleNewNotification({
                    ...details,
                    message: data.message || `From ${data.name} regarding "${data.purpose}"` // Handle different data shapes
                });
            });
        });
