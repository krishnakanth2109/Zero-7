...
export default function AdminManageCandidates() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showAlert('error', error.response?.data?.message || 'Failed to save candidate. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        try {
          ...
          const formattedData = data.map((item, index) => {
            ...
            return {
              ...
              exp: item['Experience (Years)'] ? Number(item['Experience (Years)']) : 0,
          ...
          for (const [index, candidateData] of formattedData.entries()) {
              try {
                  ...
                  errorCount++;
                  errors.push(`Row ${index + 2}: ${candidateData.email} - ${error.response?.data?.message || 'Failed to create'}`);
              }
          }

          showAlert('success', `📊 Import completed: ${successCount} successful, ${errorCount} failed`);
          if (errors.length > 0) {
    ...
    <div className='min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8 font-sans'>
      <div className='max-w-7xl mx-auto bg-white rounded-xl shadow-lg p-6 sm:p-8'>
        ...
        {alertMessage.message && (
          <div className={`mb-6 p-4 flex items-center rounded-lg shadow-sm animate-fade-in 
            ${alertMessage.type === 'success' ? 'bg-green-100 border border-green-400 text-green-700' : 'bg-red-100 border border-red-400 text-red-700'}`} 
            role="alert">
            {alertMessage.type === 'error' ? <XCircle className="w-5 h-5 mr-3 flex-shrink-0" /> : <div className="w-5 h-5 mr-3 flex-shrink-0 text-lg font-bold">✓</div>}
        ...
        <div className='bg-white rounded-lg shadow-md border border-gray-200'>
          <div className='p-5 border-b border-gray-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4'>
            ...
            <div className='flex flex-wrap items-center gap-3'>
                {/* Import Data */}
                <label  className='flex items-center px-4 py-2 bg-purple-500 text-white rounded-lg hover:bg-purple-600 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition duration-200 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium'>
                    {importing ? (
                ...
                {/* Export Data */}
                <button className='flex items-center px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 transition duration-200 text-sm font-medium' onClick={exportToExcel}>
                  <Download size={18} className='mr-2' /> Export Data
                </button>
                
                {/* Add New Candidate */}
                <button className='flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition duration-200 text-sm font-medium' onClick={handleOpenAddModal}>
                  ...
                  <tr>
                    <td colSpan='8' className='px-6 py-10 text-center text-gray-500'>
                      <div className="flex flex-col items-center justify-center">
                          ...
                          <span className="text-lg font-medium">No candidates found. Add one using the form above.</span>
                      </div>
                    </td>
                  </tr>
                ) : (
          ...
          <div className="p-5 border-t border-gray-200 bg-gray-50 flex flex-col sm:flex-row items-center justify-between text-sm text-gray-600">
            <div className="mb-2 sm:mb-0">
                Showing <strong className="font-semibold">{candidates.length}</strong> of{' '}
                <strong className="font-semibold">{candidates.length}</strong> candidates
            </div>
          </div>
        </div>
      </div>
