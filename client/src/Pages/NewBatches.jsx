...
const NewBatches = () => {
    ...
    const RegistrationModal = ({ isOpen, onClose, initialCourse }) => {
        const [formData, setFormData] = useState({
            name: '', email: '', phone: '', selectedCourse: initialCourse || '',
            programType: '', message: ''
        });
                                ...
                                {filteredCourses.length > 0 ? (
                                    filteredCourses.map((course) => (
                                        <tr key={course._id}>
                                            ...
                                            <td><button className="register-btn" onClick={() => handleRegister(course.name)}>Register Now</button></td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr><td colSpan="6" className="no-results">No courses found matching your search.</td></tr>
                                )}
                            </tbody>
                        </table>
                    )}
