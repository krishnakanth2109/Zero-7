    ...
    <div className="bench-page">
      ...
      <section className="hero-section">
        <div className="overlay">
          ...
          <button className="btn-gradient" onClick={() => setShowForm(true)}>Register Now</button>
        </div>
      </section>
      <section className="registration-section">
        <h2>Resume Marketing Registration</h2>
        <p>Join our <strong>Bench Program</strong> to get placed faster! Our management will inform you about the fees for resume marketing. Fill the form below to connect with us directly.</p>
        <button className="btn-gradient" onClick={() => setShowForm(true)}>Connect with Management</button>
      </section>
            ...
            <tbody>
              {candidates.map((c) => (
                <tr key={c._id}> {/* Use _id from MongoDB */}
                  ...
                  <td>
                    <button className="btn-gradient" onClick={() => handleGetDetails(c)}>
                      Get Details
                    </button>
                  </td>
                </tr>
      ...
      <section className="testimonials">
        ...
        <div className="testimonial-cards">
          {testimonials.map((t, i) => (
            <div className="testimonial-card" key={i}><img src={t.img} alt={t.name} className="testimonial-img"/><p>"{t.text}"</p><h4>- {t.name}</h4></div>
          ))}
        </div>
      </section>
        ...
        <div className="popup-form-overlay">
          <div className="popup-form">
            <form onSubmit={handleRequestSubmit}>
              <h2>Please provide your company details to request info for: {selectedCandidate.name}</h2>
              <input name="companyName" placeholder="Company Name" onChange={handleRequestFormChange} required />
              <input name="website" placeholder="Website / LinkedIn Profile (optional)" onChange={handleRequestFormChange} />
              <input name="contactPerson" placeholder="Contact Person Name" onChange={handleRequestFormChange} required />
              <input name="designation" placeholder="Designation / Role" onChange={handleRequestFormChange} required />
              <input type="email" name="email" placeholder="Official Email ID" onChange={handleRequestFormChange} required />
              <input name="phone" placeholder="Phone Number (with WhatsApp)" onChange={handleRequestFormChange} required />
              ...
              <input name="budget" placeholder="Budget / CTC Range (optional)" onChange={handleRequestFormChange} />
              <textarea name="notes" placeholder="Additional Notes / Comments (optional)" onChange={handleRequestFormChange} />
              ...
              <div className="form-actions">
                ...
                <button type="button" onClick={() => setSelectedCandidate(null)} className="btn-secondary">Cancel</button>
              </div>
            </form>
          </div>
        </div>
