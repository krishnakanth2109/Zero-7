    ...
    <div className="p-4 sm:p-6 bg-gray-50 min-h-full">
      <div className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-lg p-6 mb-8 flex items-center justify-between text-white">
        <div className="flex items-center space-x-4">
          <div className="bg-white bg-opacity-20 p-3 rounded-full"><ClipboardList size={28} /></div>
          <div>
            <h3 className="text-2xl sm:text-3xl font-bold">Batch Demo Enrollments</h3>
            <p className="text-blue-200 text-sm">Registrations from the 'New Batches' page</p>
          </div>
        </div>
                  ...
                  enrollments.map((item) => (
                    <tr key={item._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <button onClick={() => handleDelete(item._id)} className="text-red-600 hover:text-red-800" title="Delete Enrollment"><Trash2 size={18} /></button>
                      </td>
                    </tr>
                  ))
                ) : (
