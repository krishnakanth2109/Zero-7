...
const BenchList = () => {
  ...
  const [enrollmentFormData, setEnrollmentFormData] = useState({
    name: '', contact: '', email: '', location: '', resume: '',
  });
  const [isEnrolling, setIsEnrolling] = useState(false);

  // --- CORRECTED: Handlers for "Candidate Enrollment" form ---
  ...
  const handleRequestSubmit = async (e) => {
    ...
    try {
      const payload = { ...requestFormData, candidateName: selectedCandidate.name };
    ...
    <div className='bench-page'>
      ...
      <section className='form-section'>
        ...
        <form onSubmit={handleEnrollmentSubmit} className='enrollment-form'>
          ...
          <input type='text' name='resume' placeholder='Your Resume Link (Google Drive, etc.)' value={enrollmentFormData.resume} onChange={handleEnrollmentChange} required pattern='https?://.+' title='Enter a valid URL' />
      ...
      <section className='stats-section'>
        <div className='bg-[linear-gradient(45deg,#0f8afe,#00bfff)] rounded-xl flex-1 p-6 shadow-xl min-w-[200px]'><h2>{stats.candidates}+</h2><p>Candidates Available</p></div>
        <div className='bg-[linear-gradient(45deg,#0f8afe,#00bfff)] rounded-xl flex-1 p-6 shadow-xl min-w-[200px]'><h2>{stats.clients}+</h2><p>Clients Served</p></div>
        <div className='bg-[linear-gradient(45deg,#0f8afe,#00bfff)] rounded-xl flex-1 p-6 shadow-xl min-w-[200px]'><h2>{stats.placements}+</h2><p>Placements Done</p></div>
      </section>
      ...
      <section className='candidates-section'>
        ...
        <div className='candidate-table-wrapper'>
          <table className='candidate-table'>
            <thead><tr><th>Name</th><th>Role</th><th>Skills</th><th>Experience</th><th>Location</th><th>Action</th></tr></thead>
            <tbody>
              {candidates.map((c) => (
                <tr key={c._id}>
                  ...
                  <td>{c.exp} Years</td>
                  <td>{c.location}</td>
                  <td><button className='btn-gradient' onClick={() => setSelectedCandidate(c)}>Request Info</button></td>
                </tr>
              ))}
            </tbody>
          </table>
