...
export default function ManageBlogs() {
  ...
  // --- HELPER: Resets the form to its initial state ---
  const resetForm = () => {
    setTitle('');
    setDescription('');
    setImageUrl('');
  ...
  // --- HANDLER: Prepares the form for editing a blog post ---
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setBlogs(blogs.filter(blog => blog._id !== id)); // Update UI immediately
          ...
          <form onSubmit={handleSubmit} className='space-y-4'>
            ...
            <div className="flex items-center space-x-2">
              <button 
                type='submit' 
                disabled={loading}
                className='flex-grow p-3 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 disabled:bg-blue-300 transition'
              >
              ...
              {editingId && (
                <button 
                  ...
                  className='p-3 bg-gray-600 text-white font-bold rounded-lg hover:bg-gray-700 transition'
                >
                  Cancel
                </button>
              )}
                  ...
                  <div className='p-4 flex flex-col flex-grow'>
                    ...
                    <div className="flex space-x-2 mt-auto">
                      <button 
                        ...
                        className='flex items-center justify-center w-full py-2 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded text-sm transition'
                      >
                        <FaEdit className='mr-1' /> Edit
                      </button>
                      <button 
                        onClick={() => handleDelete(blog._id)}
                        className='flex items-center justify-center w-full py-2 bg-red-500 hover:bg-red-600 text-white font-bold rounded text-sm transition'
                      >
