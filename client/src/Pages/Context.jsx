...
export default function Context() {
  ...
  // Fetch Placed Candidates for the Scrollbar
  useEffect(() => {
    const fetchPlaced = async () => {
      try {
        ...
        const placedCandidates = res.data.filter(
          (interview) =>
            interview.interviewLevel === 'placed' ||
            interview.status?.toLowerCase() === 'placed'
        )
  ...
  return (
    <section className='context-wrapper'>
      {/* Internal CSS for the Ticker to ensure it works immediately */}
      <style>{`
        .ticker-container {
          width: 100%;
          overflow: hidden;
          ...
          display: flex;
          align-items: center;
          margin-bottom: 2rem; /* Spacing below the ticker */
        }
        .ticker-wrapper {
          display: flex;
          white-space: nowrap;
