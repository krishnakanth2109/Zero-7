...
router.post('/', async (req, res) => {
    try {
        const { title, description, imageUrl, gallery } = req.body; // <-- ADDED gallery

        if (!title || !description || !imageUrl) {
            return res.status(400).json({ message: "Title, description, and main image are required." });
        }
        ...
        const newBlog = new Blog({
            ...
            description,
            imageUrl,
            gallery: gallery || [] // <-- ADDED gallery data
        });
...
router.put('/:id', async (req, res) => {
    try {
        const { title, description, imageUrl, gallery } = req.body; // <-- ADDED gallery
        const updatedData = { title, description, imageUrl, gallery };
