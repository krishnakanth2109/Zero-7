...
import { CheckCircle, XCircle, Eye, Trash2, Calendar, Building, User, Mail, Phone, Globe, FileText } from 'lucide-react';
...
export default function AdminViewRequests() {
    ...
    const getStatusCounts = () => {
        const pending = requests.filter(req => !req.status || req.status === 'pending').length;
                        ...
                        {requests.length === 0 ? (
                            <tr>
                                <td colSpan="7" className="empty-state">
                                    ...
                                    <p>There are no candidate information requests at the moment.</p>
                                </td>
                            </tr>
                        ) : (
                            requests.map(req => {
                                    ...
                                    <tr key={req._id} className={statusBadge.class.replace('status-', 'row-')}>
                                        ...
                                        <td>
                                            <div className="actions-cell-container">
                                                ...
                                                <button 
                                                    className={`delete-btn ${actionLoading === `delete-${req._id}` ? 'loading' : ''}`}
                                                    onClick={() => handleDelete(req._id)}
                                                    disabled={actionLoading === `delete-${req._id}`}>
                                                    <Trash2 size={14} />
                                                    {actionLoading === `delete-${req._id}` ? '' : 'Delete Request'}
                                                </button>
                                            </div>
                                        </td>
                                    </tr>
            ...
            {selectedRequest && (
                <div className="modal-overlay" onClick={() => setSelectedRequest(null)}>
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        ...
                        <p>
                            <strong><Building size={16} /> Company:</strong> 
                            {selectedRequest.companyName}
                        </p>
                        <p>
                            <strong><User size={16} /> Contact Person:</strong> 
                        ...
                        <p>
                            <strong><Globe size={16} /> Website:</strong> 
